...
First, let's semi-formally introduce symmetric monoidal category is:
...
Next, we're gonna blackbox whatever an operad is. Yes. (Also, there are many texts by Peter describing this). Anyway, a ``slogan'' for operad, is it encodes how to define $n$-fold products on some ``things'', and how to combine these products.

In the category of small categories $\Cat$ (with functors as 1-morphisms, natural transformation as 2-morphisms), we have an operad $\cP$, such that a notion of ``pseudoalgebra'' over this $\cP$, precisely describes a category $\A$ and a collection of $n$-fold products on the category. Collect tese pseudoalgebras as $\P$-$\PsAlg$.
...
\begin{thm}
    The 2-categories $\SymMon$, $\cP$-$\PsAlg$, and $\F$-$\PsAlg$ are equivalent.
\end{thm}
...
A problem is: we need to describe all the $n$-fold product and equivariant structure, before even talking about a symmetric monoidal $G$-category, and it's genuinely hard to study its structure. So what the heck is a symmetric monoidal $G$-category? If one can describe it using just binary product, it'll be a lot easier.

\hfil

As an analogy, we also have the category of based finite $G$-sets, $\F_G$, and also a way of expressing these $\cP_G$-pseudoalgebras as pseudofunctors $\F_G\rightarrow \Cat_G$ (denoted $\F_{G,\RR}$-$\PsAlg$). After having all the correspondance in non-equivariant case, one can also ask: Are these two damn categories equivalent? If so, HOW?
