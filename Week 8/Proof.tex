...
\title{Inputs of Equivariant Infinite Loop Space Machines}
\author{Zih-Yu Hsieh}
\date{\today}
...
% and establish a 2-functor $\RR_G$ generating $\F_G$-pseudoalgebra from $\cP_G$-pseudoalgebra, relating the two.
...
\subsection{2-Category $\F_G$-$\PsAlg$ (Maybe need some extra $G$-axioms on the coherent natural isomorphisms)}
...
\begin{defn}[$\F_G$-pseudoalgebra]
    ...
    \end{align*}
    is a $G$-functor. Finally, we also require the transformation for composition to be $G$-equivariant. Given $f:X\rightarrow Y$ and $\ell:Y\rightarrow Z$ in $\F_G$, the following equality holds for all $g\in G$:
    \begin{align*}
        \xymatrix{
            \B(X) \ar[r]^-{g^{-1}} & \B(X) \ar@(ur,ul)[r]^-{\B(\ell\circ f)} \ar@(dr,dl)[r]_-{\B(\ell)\circ \B(f)} \ar@{=>}[r]^-{\psi_{f,\ell}} & \B(Z) \ar[r]^-{g} & \B(Z)
        } \quad=\quad \xymatrix{
            \B(X) \ar@(ur,ul)[rr]^-{\B(g\ell g^{-1}\circ gfg^{-1})} \ar@(dr,dl)[rr]_-{\B(g\ell g^{-1})\circ \B(gfg^{-1})} \ar@{=>}[rr]^-{\psi_{gfg^{-1},g\ell g^{-1}}} && \B(Z)
        }
    \end{align*}
\end{defn}
...
Define $\B:\F_G\rightarrow \Cat_G$ given by $\B(\mathbf{n}^\alpha):=\A^{\mathbf{n}^\alpha}$ (with underlying category $\A^n$, a discrete category with $2^n$ objects), and for any morphism $f:\mathbf{n}^\alpha\rightarrow\mathbf{m}^\beta$, define $\Bf:\A^{\mathbf{n}^\alpha}\rightarrow\A^{\mathbf{m}^\beta}$ by 
\begin{align}
    \Bf\left((a_i)_{i=1,...,n}\right):= \left(\sum_{i\in f^{-1}(j)}a_i\ \mathrm{mod}\ 2 \right)_{j=1,...,m},
\end{align}
for special case where $f^{-1}(j)=\emptyset$, define the $j$th entry as $0$.

To check it preserves composition up to coherent natural isomorphism, consider $\mathbf{n}^\alpha\xrightarrow{f}\mathbf{m}^\beta\xrightarrow{g}\mathbf{k}^\gamma$ in $\F_G$, then $\B(g\circ f):\B^{\mathbf{n}^\alpha}\rightarrow\B^{\mathbf{m}^\beta}$ is given as follow:
\begin{align}
    \B(g\circ f)((a_i)_{i=1,...,n}) &= \left(\sum_{i\in (g\circ f)^{-1}(\ell)}a_i\ \mathrm{mod}\ 2\right)_{\ell=1,...,k}\\
    ...
    &= \Bg\left(\left(\sum_{i\in f^{-1}(j)}a_i\ \mathrm{mod}\ 2\right)_{j=1,...,m}\right)\\
    &= \Bg\left(\Bf((a_i)_{i=1,...,k})\right)
\end{align}
(Here, the well-defineness of the function heavily relies on the assumption that if $f^{-1}(j)=\emptyset$, the corresponding sum is $0$). In particular, $\B(g\circ f) = \Bg\circ \Bf$, so $\B$ is a functor (or an $\F_G$-algebra), and since each $\B(\mathbf{n}^\alpha):= \A^{\mathbf{n}^\alpha}$, it's in fact an $\F_{G,\RR}$-algebra.
...
\bibitem{CG14}
A. S. Corner, N. Gurski.
Operads with gerneral groups of equivariance, and some 2-categorical aspects of operads in $\Cat$.
