...
\section{Motivating Examples: Natural Transformations and Homotopies}
...
\begin{defn}
    ...
    \textbf{II. Horizontal Composition:} For each 0-cells $X,Y,Z\in\mathcal{B}_0$, here exists a \emph{horizontal compositor functor}
    ...
    \[\xymatrix{
        \text{*}\times \mathcal{B}(X,Y) \ar[rr]^-{\eta_Y\times \Id_{\mathcal{B}(X,Y)}} && \mathcal{B}(Y,Y)\times\mathcal{B}(X,Y)\ar[rr]^-{c_{XYY}} \ar@{}[d]^(.25){}="a"^(.75){}="b" \ar@{=>}"a";"b"^{\ell_{XY}} && \mathcal{B}(X,Y)\\
        \text{*}\times \mathcal{B}(X,Y) \ar[rrrr]^-{\Id_{\mathcal{B}(X,Y)}} &&&& \mathcal{B}(X,Y)\\
        \mathcal{B}(X,Y)\times \text{*} \ar[rr]^-{\Id_{\mathcal{B}(X,Y)}\times \eta_X} && \mathcal{B}(Y,X)\times\mathcal{B}(X,X)\ar[rr]^-{c_{XXY}} \ar@{=>}[u]_-{r_{XY}} && \mathcal{B}(X,Y)
    }\]
...
\begin{exmp}
    ...
    Also, if consider the spaces of based maps $F(X,Y)$, notice given $s_X:*\rightarrow X$ and $s_Y:*\rightarrow Y$ two structure maps, it satisfies the following fibre product property:
    \[\xymatrix{
        Z \ar@(r,u)[rrd] \ar@(d,l)[ddr] \ar@{-->}[dr]\\
        & F(X,Y) \ar@{^{(}->}[r]^{\iota} \ar@{->>}[d] & \Hom_\Top(X,Y) \ar[d]^-{\_\circ s_X}\\
        & \Hom_\Top(\text{*},\text{*}) \ar[r]_-{s_Y\circ \_} & \Hom_\Top(\text{*},Y)
    }\]
\end{exmp} 

\hfil

With the examples in mind, we can construct the generalization:
...
\begin{con}
    ...
    \[\xymatrix{
        X\sqcup Y \ar[r] \ar[d] & X\times Y \ar[d] \ar@(r,u)[ddr]\\
        \text{*} \ar@(d,l)[drr] \ar[r] & X\wedge Y \ar@{-->}[dr]\\
        && Z
    }\]
    %Which, this construction (using the universality) should be associative up to isomorphism.
    Here, need to emphasize that this construction is not necessarily symmetric monoidal (as there are counterexamples in $\Top$ that violates associativity). But, once this is associative (up to isomorphism), the universal property establishes $\_\wedge\_:\mathcal{V}_*\times\mathcal{V}_*\rightarrow\mathcal{V}_*$ as a functor. Moreover, the universal property can be used to establish an internal hom functor $\mathcal{V}_*(\_,\_):\mathcal{V}_*\times\mathcal{V}_*\rightarrow\mathcal{V}_*$ as the fibre product diagram:
    \[\xymatrix{
        Z \ar@(r,u)[drr] \ar@{-->}[dr] \ar@(d,l)[ddr]\\
        & \mathcal{V}_*(v,u) \ar[d] \ar[r] & \mathcal{V}(v,u) \ar[d]\\
        & \mathcal{V}(\text{*},\text{*}) \ar[r] & \mathcal{V}(\text{*},u)
    }\]
\end{con}
