...
Finally, there is a natural right $\Sigma_j$-action on $\mathcal{E}_X (j)$, by pre-permuting the input entries. Which, permutation in $\mathcal{E}_X (k)$ is equivalent to ``permuting orders'' of blocks $(j_1,...,j_k)= \{\{1,2,...,j_1\},...,\{j_1+...+j_{k-1}+1,...,j_1+...+j_{k-1}+j_k\}\}$  in $\{1,...,j\}$, and each $\Sigma_{j_s}$-action on $\mathcal{E}_X (j_s)$ is equivalent to a permutation only on each block of $(j_1,...,j_k)$.
...
If $\mathcal{C}(0)=\kappa$ (with $\gamma:\mathcal{C}(0)\rightarrow \mathcal{C}$ being $\id_{\mathcal{C}(0)}$), the operad is called \emph{unital operad}.
\end{defn}
...
Also, each identity morphism $\kappa_\psi\rightarrow \kappa$ induces a trivialization $t:\kappa[\Sigma_j]\rightarrow\kappa$. Combining the action $\sigma\in \Sigma_k$ (by identifying conatination of blocks $(j_{\sigma^{-1}(1)},...,j_{\sigma^{-1}(k)})=\{1,...,j\}$ in order, also inducing inclusion $\Sigma_{j_s}\hookrightarrow \Sigma_j$), we get the following morphism:
...
\subsection{Topological Example: Stasheff Associahedra}
