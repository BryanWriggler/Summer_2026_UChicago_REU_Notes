...
\begin{proof}
    ...
    If fix $\mathcal{B}$ and varies $\mathcal{A},\mathcal{C}$, since everything is constructed functorially, let's skip this part :)

    \hfil

    \textbf{IV. Equivariance Version:}

    Assuming the first three statements, the remaining statement is to show: $\Phi$ as a functor commutes with $G$-action.
\end{proof}
