...
\begin{defn}
Given symmetric monoidal category $\mathcal{V}$(use $\kappa$ to denote unit), a \emph{planar operad} $C$ in $\mathcal{V}$ is a collection of objects $\{C(j)\in\mathcal{V}\}_{j\in\NN}$, a \emph{unit map} $\eta:1\rightarrow C(1)$, together with composition maps
...
Alternatively, one can let each $C(j_s)$ act on $\bigtensor_{q=1}^{j_s}C(i_{g_{(s-1)}+q})$
