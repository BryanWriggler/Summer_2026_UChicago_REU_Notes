...
\begin{exmp}
    A nontrivial example, we can consider $\mathcal{M}(j)=\kappa[\Sigma_j]:=\coprod_{\sigma\in\Sigma_j}\kappa$, coproduct of $\#\Sigma_j$ copies of unit $\kappa$ (so $\mathcal{M}(0)=\mathcal{M}(1)=\kappa$). There is a natural $\Sigma_j$-action on $\mathcal{M}(j)$, by permuting the index of coproducts according to right multiplication (so, index $\psi$ maps to index $\psi \sigma$, when acted by $\sigma$).

    To define the operad morphisms, notice the following structure: Given any object $A,B\in\mathcal{V}$, 
\end{exmp}

\newpage

\subsection{Operad: Alternative Definition and Relation to Monad}
